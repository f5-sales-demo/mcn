...
for relative in terraform/versions.tf terraform/modules/xc-site/versions.tf coverage/smsv2/versions.tf; do
  ...
  if printf '%s\n' "$block" | grep -Eq 'version[[:space:]]*=[[:space:]]*"= 9\.2\.1"'; then
    ok "${relative} pins = 9.2.1"
  else
    bad "${relative} does not pin exactly = 9.2.1"
  fi
...
for relative in .github/workflows/terraform.yml prompt.txt docs/en/demo/deploy.mdx \
  ...
  if grep -Fq '9.2.1' "${REPO_ROOT}/${relative}"; then
    ok "${relative} references v9.2.1"
  else
    bad "${relative} is missing the v9.2.1 reference"
  fi
done
