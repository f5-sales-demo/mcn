...
echo "2. every xcsh consumer pins exactly v7.3.0"
for relative in terraform/versions.tf terraform/modules/xc-site/versions.tf coverage/smsv2/versions.tf; do
  ...
  if printf '%s\n' "$block" | grep -Eq 'version[[:space:]]*=[[:space:]]*"= 7\.3\.0"'; then
    ok "${relative} pins = 7.3.0"
  else
    bad "${relative} does not pin exactly = 7.3.0"
  fi
...
for relative in .github/workflows/terraform.yml prompt.txt docs/en/demo/deploy.mdx \
  ...
  if grep -Fq '7.3.0' "${REPO_ROOT}/${relative}"; then
    ok "${relative} references v7.3.0"
  else
    bad "${relative} is missing the v7.3.0 reference"
  fi
done
