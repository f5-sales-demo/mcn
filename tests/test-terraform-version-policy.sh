...
echo "2. every xcsh consumer pins exactly v9.4.0"
for relative in terraform/versions.tf \
  ...
  if printf '%s\n' "$block" | grep -Eq 'version[[:space:]]*=[[:space:]]*"= 9\.4\.0"'; then
    ok "${relative} pins = 9.4.0"
  else
    bad "${relative} does not pin exactly = 9.4.0"
  fi
...
for relative in .github/workflows/terraform.yml prompt.txt docs/en/demo/deploy.mdx \
  ...
  if grep -Fq '9.4.0' "${REPO_ROOT}/${relative}"; then
    ok "${relative} references v9.4.0"
  else
    bad "${relative} is missing the v9.4.0 reference"
  fi
done
