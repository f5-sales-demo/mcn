...
require_text terraform/versions.tf 'version = "= 9.2.1"'
...
require_text terraform/aws_vpc.tf 'resource "aws_instance" "origin"'
require_text terraform/aws_vpc.tf 'resource "aws_security_group" "origin"'
require_text terraform/aws_vpc.tf 'iam_instance_profile        = aws_iam_instance_profile.workload[0].name'
require_text terraform/aws_xc.tf 'aws_instance.origin[0].public_ip'
require_text terraform/aws_vpc.tf 'mcn-smsv2-aws-origin'
require_text terraform/aws_vpc.tf 'dnf install -y nginx amazon-ssm-agent'
require_text terraform/aws_vpc.tf 'systemctl enable --now amazon-ssm-agent'
require_text terraform/aws_vpc.tf '"5.182.212.0/22"'
require_text terraform/aws_vpc.tf '"103.135.56.0/22"'
require_text terraform/aws_vpc.tf '"159.60.128.0/18"'
require_text terraform/aws_vpc.tf '"185.94.140.0/22"'
reject_text terraform/aws_vpc.tf 'aws_origin_diagnostic_open_ingress'
require_text terraform/aws_vpc.tf 'cidr_blocks = [var.aws_workload_vpc_cidr]'
require_text terraform/aws_vpc.tf 'cidr_blocks = ["${aws_instance.workload[0].public_ip}/32"]'
require_text terraform/aws_vpc.tf 'cidr_blocks = [var.aws_vpc_cidr]'
require_text terraform/aws_vpc.tf 'for ip in aws_eip.ce[*].public_ip : "${ip}/32"'
...
require_text scripts/aws-smsv2-uat-preflight.sh 'aws_origin_public_ip'
