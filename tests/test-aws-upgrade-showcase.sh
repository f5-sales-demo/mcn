...
require_text terraform/aws_vpc.tf 'target_id         = aws_network_interface.sli[each.value.index].private_ip'
require_text terraform/aws_vpc.tf 'availability_zone = "all"'
require_text terraform/aws_vpc.tf 'resource "aws_lb_listener" "smsv2" {'
require_text terraform/aws_vpc.tf 'for_each = var.enable_aws_tgw_connect ? toset(aws_network_interface.sli[*].private_ip) : toset([])'
