...
resource "terraform_data" "aws_tgw_contract_gate" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "Provider v9.2.3 must expose the exact immutable SMSv2 API v7.0 contract."
    }
    precondition {
      condition = (
      ...
      error_message = "Provider v9.2.3 must publish all and only the required SMSv2 capabilities as available."
    }
    precondition {
      condition = (
