...
resource "terraform_data" "aws_tgw_contract_gate" {
  ...
  lifecycle {
    ...
    precondition {
      ...
      error_message = "Provider v9.4.0 must expose the exact immutable SMSv2 API v7.0.6 contract."
    }
    precondition {
      condition = (
      ...
      error_message = "Provider v9.4.0 must publish all and only the required SMSv2 capabilities, including evidence-backed AWS node configuration, as available."
    }
    precondition {
      condition = try(
