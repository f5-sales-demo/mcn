...
# The CE OS disk must be explicit and retain capacity headroom for the current
# advertised platform payload.
run "ce_os_disk_is_sized_for_the_advertised_versions" {
  ...
  assert {
    ...
    error_message = "The CE OS disk size must be set explicitly."
  }

  assert {
    condition     = azurerm_linux_virtual_machine.this.os_disk[0].disk_size_gb >= 40
    error_message = "The CE OS disk must be at least 40 GB."
  }
}
