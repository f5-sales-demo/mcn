...
description: The five vifdump commands that capture packets on a Customer Edge's virtual interfaces. Documented from verified catalog metadata and never run here, because they write capture files onto the node — including an account of what happened when one was invoked as a harmless-looking probe.
sidebar:
  order: 30
