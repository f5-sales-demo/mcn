...
description: Human-readable, minimal engineering specification derived from the source-of-truth prompt for the Active/Active eBGP/ECMP F5 XC Customer Edge deployment on Azure.
sidebar:
  order: 26
...
It is derived directly from the project's source-of-truth prompt (`prompt.txt`) created via automated prompt engineering.
...
- **Backend**: Partial Azure Blob storage backend (`backend "azurerm" {}`).

### Tenant Guard & CIDR Safety Checks (`data.tf`, `main.tf`)
- **Tenant Guard**: Script `scripts/xc-env-tenant.sh` inspects ambient `XCSH_API_URL` and returns JSON. A `lifecycle { postcondition }` block on `data.external.xc_env_tenant` asserts the ambient tenant matches `var.expected_xc_tenant` (`f5-sales-demo`), failing the plan if an operator is pointed at the wrong tenant.
...
- **VPM Configuration**: Writes `/etc/vpm/config.yaml` (`0600`, `root:root`) containing:
  - `ClusterType: ce`, `ClusterName: ${cluster_name}`, `Token: ${token}`
...
- 3 NICs per VM (mgmt/SLO with static private IP, external dynamic, internal dynamic) with `ip_forwarding_enabled = true` and `accelerated_networking_enabled = false`.
- `azurerm_linux_virtual_machine`: Uses marketplace image `volterraedgeservices/volterra-node/volterra-node` with required `plan` block, 64 GB `StandardSSD_LRS` OS disk, and `custom_data = base64encode(cloud_init)`.

### F5 XC Site & BGP Binding (`modules/xc-site`)
- **XC Site**: `xcsh_securemesh_site_v2` with explicit `eth0` SLO interface bound to mgmt NIC MAC.
...
- **Registration Approval**: `data.xcsh_site_registration` resolves runtime `r-<uuid>` registration, approved automatically by `xcsh_registration_approval`.
- **eBGP Peering**: `xcsh_bgp` configures CE ASN `64512` to Azure Route Server ASN `65515` on port 179 bound to the explicit interface `var.interface_name`.
...
- **`modules/client-vm`**: Test client Ubuntu VM in `snet-hub-internal` with NSG and public IP.
...
- **Registration Token**: Tenant-scoped `xcsh_token.ce` (`mcn-ce-registration` in `system`).
- **Namespace Lookup**: Read-only lookup `data.xcsh_namespace.mcn` (`multi-cloud-networking`, `namespace = ""`).
...
- **Canadian Virtual Sites**:
  - `xcsh_virtual_site.canada_re`: Targets Regional Edge cities `toronto` and `montreal`.
...
Exports all operational metrics required for verification without hardcoding prose values:
