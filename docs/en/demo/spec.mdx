...
- **Providers**:
  - `f5-sales-demo/xcsh` (`= 9.3.0`): `api_url` is dynamically derived from `var.expected_xc_tenant` (`https://${var.expected_xc_tenant}.console.ves.volterra.io`).
...
- **Base Image & Storage**: `libvirt_volume.base_cloud` imports only the F5-issued KVM appliance URL from `xcsh_site_image`; `libvirt_volume.ce_disk` provisions 20 GB root overlay disks per CE (`onprem-ce-01`, `02`, `03`). The image query requires exactly one valid Sales Demo tenant `maurice_config`.
  With v9.3.0, it reports `External tenant prerequisite unavailable` before any apply when that externally owned prerequisite is absent.
