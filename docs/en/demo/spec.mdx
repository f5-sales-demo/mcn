...
six Connect peers, and twelve BGP sessions. [PR #1103](https://github.com/f5-sales-demo/mcn/pull/1103)
...
- **Providers**:
  - `f5-sales-demo/xcsh` (`= 9.2.3`): `api_url` is dynamically derived from `var.expected_xc_tenant` (`https://${var.expected_xc_tenant}.console.ves.volterra.io`).
...
- **Base Image & Storage**: `libvirt_volume.base_cloud` imports only the F5-issued KVM appliance URL from `xcsh_site_image`; `libvirt_volume.ce_disk` provisions 20 GB root overlay disks per CE (`onprem-ce-01`, `02`, `03`). The image query requires exactly one valid Sales Demo tenant `maurice_config`.
  With v9.2.3, it reports `External tenant prerequisite unavailable` before any apply when that externally owned prerequisite is absent.
...
- **F5 XC Site & eBGP**: `xcsh_securemesh_site_v2.onprem_kvm` (`onprem-kvm-site`) and `xcsh_bgp.onprem_ebgp` peer CE ASN `64512` to the Terraform-owned FRR router (`10.100.0.2`, ASN `65515`) on port 179 over `eth0`; `10.100.0.1` remains the libvirt NAT gateway.
