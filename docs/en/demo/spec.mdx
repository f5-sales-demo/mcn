...
and is formatted so that it functions as both an architectural specification and an execution prompt for `xcsh -p`.
...
- **Tenant Guard**: Executable script `scripts/xc-env-tenant.sh` (`chmod +x`) inspects ambient `XCSH_API_URL` and outputs JSON `{"tenant":"<label>","api_url_set":"<bool>"}`. A `lifecycle { postcondition }` block on `data.external.xc_env_tenant` asserts the ambient tenant matches `var.expected_xc_tenant` (`f5-sales-demo`), failing the plan if an operator is pointed at the wrong tenant.
