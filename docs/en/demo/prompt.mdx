...
1. **Versions & Providers** (`versions.tf`, `providers.tf`):
   ...
   - Providers: `f5-sales-demo/xcsh` (`= 9.4.0`, setting `api_url = local.xc_api_url` derived from
...
| --- | --- |
| `f5-sales-demo/xcsh = 9.4.0` | Provides the immutable SMSv2 contract, evidence-backed AWS node configuration, Azure NIC MAC-read stability, the pre-mutation Azure Route Server multihop capability gate, and the actionable KVM image tenant-prerequisite diagnostic. |
