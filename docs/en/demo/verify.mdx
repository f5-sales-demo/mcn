...
The current Sales Demo tenant prerequisite is exactly one valid `maurice_config`;
when the image data source reports that it is not one, stop before mutation and
repair the tenant-owned prerequisite before retrying the same plan.
