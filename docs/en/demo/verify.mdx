...
4. **The VIPs serve traffic.** From the client VM, and **with the `Host` header** — see the
   second trap. Test both the Rest of World (`f5-sales-demo.com`) and Canada (`f5-sales-demo.ca`) domains.
   ...
   For the Canada Regional path (`f5-sales-demo.ca`):
   ...
   az vm run-command invoke -g "$RG" -n "$CLIENT" \
     ...
     --scripts "ok=0; fail=0
                ...
                echo \"CANADA_DOMAIN_OK=\$ok CANADA_DOMAIN_FAIL=\$fail\""
   ```
